...
class RemoteDataSource {
    ...
    fun getMovieCredit(movieID: String, callback: MovieCreditsCallback) {
        EspressoIdlingResource.increment()

        callback.movieCreditCallback(ResponseHelper.Loading())
        ApiConfig.getClient().getMovieCredits(movieID)
            .enqueue(object : Callback<MovieCreditsResponse> {
                ...
                override fun onFailure(call: Call<MovieCreditsResponse>, t: Throwable) {
                    EspressoIdlingResource.decrement()
                    callback.movieCreditCallback(ResponseHelper.Error(t.message.toString()))
                }

            })
    }

    fun getShowCredit(movieID: String, callback: MovieCreditsCallback) {
        EspressoIdlingResource.increment()

        callback.movieCreditCallback(ResponseHelper.Loading())
        ApiConfig.getClient().getShowCredits(movieID)
            .enqueue(object : Callback<MovieCreditsResponse> {
                override fun onResponse(
                    call: Call<MovieCreditsResponse>,
                    response: Response<MovieCreditsResponse>
