...
class MoviePagedListAdapter :
    PagedListAdapter<MovieLocalEntity, MoviePagedListAdapter.MyViewHolder>(DIFF_CALLBACK) {

    private lateinit var myInterface: MovieInterface

    fun setInterface(newInterface: MovieInterface) {
        this.myInterface = newInterface
    }

    inner class MyViewHolder(private val binding: ItemMovieBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(model: MovieLocalEntity) {
            binding.apply {
    ...
    companion object {
        private val DIFF_CALLBACK: DiffUtil.ItemCallback<MovieLocalEntity> =
            object : DiffUtil.ItemCallback<MovieLocalEntity>() {
                override fun areItemsTheSame(
                    old: MovieLocalEntity,
                    new: MovieLocalEntity
                ): Boolean {
                    return old == new
                }
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        holder.bind(getItem(position) as MovieLocalEntity)
    }

    interface MovieInterface {
        fun onclick(model: MovieLocalEntity)
        fun share(model: MovieLocalEntity)
    }
}